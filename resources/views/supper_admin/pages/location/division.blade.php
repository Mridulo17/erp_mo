...
@section('title', config('app.name') . ' - country')
...
<div class="box">
    ...
    <div class="box-header with-border d-flex justify-content-between align-items-center">
        <div>
            ...
            <h6 class="box-subtitle">This is all Divisions List</h6>
        </div>
    ...
    @include('supper_admin.components.location.division_modal')
    ...
    <div class="box-body">
        <div class="table-responsive">
            <table id="customDataTable" style="table-layout: fixed; width: 100%;" class="table table-bordered table-hover display nowrap margin-top-10 w-p100">
                ...
                <tbody>
                    @foreach($divisions as $key =>$serivice)
                    <tr>
                        <td>
                            <div class="btn-group">
    ...
    @section('script')
        <script>
            function fetchDivisions() {
                $.ajax({
                    url: '{{ route("supper_admin.divisions.index") }}',
                    type: 'GET',
            ...
            $(document).ready(function () {
                ...
                $('#divisionFrom').on('submit', function (e) {
                    ...
                    let isEdit = $('#division_id').val() !== ''; 
                    let formData = new FormData(this); 
                    let id = $('#division_id').val(); 
                    let url = isEdit 
                        ? `{{ route('supper_admin.divisions.update', ['division' => '__id__']) }}`.replace('__id__', id) 
                        : `{{ route('supper_admin.divisions.store') }}`;
                ...
                $(document).on('click', '.editBlogButton', function () {
                    ...
                    const url = '{{ route("supper_admin.divisions.edit", ":id") }}'.replace(':id', id);
                ...
                $(document).on('click', '.deletecountryBtn', function () {
                    const id = $(this).data('id');
                    const url = '{{ route("supper_admin.divisions.destroy", ":id") }}'.replace(':id', id);
